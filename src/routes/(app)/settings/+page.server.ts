...
export const load: PageServerLoad = (async ({ locals }) => {
  ...
  try {
    ...
    const [schedule_settings] = await db.select({ settings: table.schedule_settings.settings }).from(table.schedule_settings).where(eq(table.schedule_settings.scheduleId, schedule.schedule_id));
    return { user, schedule_settings: schedule_settings.settings };
  } catch (error) {
