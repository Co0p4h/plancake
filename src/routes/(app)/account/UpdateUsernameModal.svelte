...
>
  <form method="POST" action="?/updateUsername" class="space-y-3 p-6" use:enhance={enhanceUpdateUsernname} autocomplete="off">
    ...
    <button
      type="submit"
      disabled={isUpdating}
      class="focus:shadow-outline justify-end rounded bg-purple-default px-4 py-2 font-bold text-white hover:bg-purple-dark focus:outline-none cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed flex items-center"
    >
      {#if isUpdating}
